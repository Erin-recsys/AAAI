...
class LightGCN(nn.Module):
    ...
    def __init_weight(self):
        ...
        try:
            file_path = f"./data/{self.dataset.path.split('/')[-1]}/counterfactual_{self.config['top_k']}_{self.config['top_m']}.txt"
            print(file_path)
    ...
    def getUsersRating(self, users):
        ...
        if not self.training and hasattr(self, 'special_interactions') and self.special_interactions:
            ...
            if batch_indices:
                ...
                special_mask[batch_indices, item_indices] = self.config['counterfactual_popularity_weight']+0.25
